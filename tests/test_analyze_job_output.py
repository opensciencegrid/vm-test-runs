...
class TestAnalyzeJobOutput(unittest.TestCase):
    ignored_failures = [('test_04_trace', 'test_55_condorce', date.today(), date.today()),
                      ('test_05_pbs_trace', 'test_55_condorce', date.today(), date.today())]

    def run_parse_log(self, log_file, expected_status, expected_failures):
        test_log = open(log_file, 'r')
        contents = test_log.read()
        test_log.close()
        status, failures = analyze_job_output.parse_log(contents, self.ignored_failures)
    ...
    def test_passing_status(self):
        self.run_parse_log('pass.log', 'pass', [])
    ...
    def test_fail_status(self):
        ...
        self.run_parse_log('fail.log', 'fail', expected_failures)
    ...
    def test_install_status(self):
        ...
        self.run_parse_log('install.log', 'install', expected_failures)

    def test_update_status(self):
        expected_failures = ['special_install|test_05_update_packages|TestInstall|FAIL|-']
        self.run_parse_log('update.log', 'update', expected_failures)
    ...
    def test_ignored(self):
        ...
        self.run_parse_log('ignore.log', 'ignored', expected_failures)

    def test_cleanup_status(self):
        expected_failures = ['special_cleanup|test_03_remove_packages|TestCleanup|FAIL|-']
        self.run_parse_log('cleanup.log', 'cleanup', expected_failures)
